...
1. 当前状态
   机床名称、IP、端口、在线状态、当前机床状态、控制器模式、自动模式、运行模式、报警状态、急停状态、当前报警信息、最近程序号、最近程序加工时长、最近开机时间、最近关机时间
2. 当日统计
   当日开机累计时长、当日运转累计时长、当日切削累计时长、当日循环累计时长、当日待机累计时长、当日报警累计时长、当日急停累计时长、当日利用率

说明：

1. `当日加工累计时长`
   当前版本已升级为优先使用 FANUC 原生 timer
2. `当日运转累计时长`
   优先使用 `cnc_rdtimer(TYPE_OPERATING)`
3. `当日切削累计时长`
   优先使用 `cnc_rdtimer(TYPE_CUTTING)`
4. `当日循环累计时长`
   优先使用 `cnc_rdtimer(TYPE_CYCLE)`
5. `当日待机累计时长`
   优先使用 `cnc_rdtimer(TYPE_FREE)`，如果机床不支持再回退为状态推导
6. `当日开机累计时长`
   优先使用 `cnc_rdtimer(TYPE_POWER_ON)`
7. `当日利用率`
   当前版本公式为：
   `当日运转累计时长 / 当日开机累计时长 * 100%`

## 第一阶段口径说明

当前版本已经进入“第一阶段：优先保证可采集和口径准确”：

1. 日累计统计改为按采集端本地时区记账，不再按 UTC 记到错误日期
2. 时间类累计值优先使用 FANUC 原生 timer 差分，不再单纯靠轮询状态推导
3. 报警信息增加了报警详情读取，优先读取报警号和报警文本
4. 程序加工时间增加了 `cnc_rdproctime` 读取，用于展示最近程序加工时间

当前仍需注意：

1. `最近开机时间` 和 `最近关机时间`
   仍然是采集程序观察到的上下线时间，不是 CNC 提供的历史开关机时间
2. 如果某台机床不支持原生 timer 或某个 timer 选项未开通，程序会自动回退到状态推导口径
...
2. GUI 在启动后会自动定时刷新，默认每 1 秒自动刷新一次最新值和日志，时间统计页按较低频率自动刷新
